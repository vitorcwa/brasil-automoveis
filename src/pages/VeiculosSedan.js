...
export default function Veiculos() {
   ...
   useEffect(()=>{
      ...
      .then(res=>{
         setData(res.data)
         setFilterRecords(res.data.filter(d=>d.Observacao.includes('SEDAN')))
      })
   ...
   const filterModelo = (event) => {
      setFilterRecords(data.filter(d=>d.Observacao.includes('SEDAN')).filter(d=>d.Modelo.toLowerCase().includes(event.target.value)));
   }

   // Filtro de Marcas
   const filterMarca = (event) => {
      setFilterRecords(data.filter(d=>d.Observacao.includes('SEDAN')).filter(d=>d.Marca.includes(event.target.value)));
   }

   // Filtro de Ano do modelo
   const filterAno = (event) => {
      setFilterRecords(data.filter(d=>d.Observacao.includes('SEDAN')).filter(d=>d.AnoModelo.toString().includes(event.target.value)));
   }
               ...
               <Grid item>
                  <Box mb={16}>
                     <Typography variant='h2' component='h1' className={classes.dark}>
                        Os melhores carros Sedan você encontra na Brasil Automóveis
                     </Typography>
                  </Box>
               </Grid>
            </Grid>
