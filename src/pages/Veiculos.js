...
export default function Veiculos() {
   ...
   const filterModelo = (event) => {
      setFilterRecords(data.filter(d=>d.Modelo.toLowerCase().includes(event.target.value)));
   }
   ...
   const filterObservacao = (event) => {
      setFilterRecords(data.filter(d=>d.Observacao.includes(event.target.value)));
   }

   // Filtro de Ano do modelo
   const filterAno = (event) => {
      setFilterRecords(data.filter(d=>d.AnoModelo.toString().includes(event.target.value)));
   }
               ...
               <Grid item>
                  <Box mb={16}>
                     <Typography variant='h2' component='h1' className={classes.dark}>
                        Encontre o carro dos seus sonhos na Brasil
                     </Typography>
                  </Box>
               </Grid>
            </Grid>
               ...
               <Grid container justifyContent='space-between'>
                  ...
                  <Grid item xs={12} md={3} className={classes.line}>
                     <Box mb={8} pr={{ xs: 0, md: 8 }}>
                        ...
                        <Box mb={8}>
                           <FormControl fullWidth>
                              <InputLabel id="observacao-select-label">Carroceria</InputLabel>
                              <Select 
                                 labelId="observacao-select-label"
                           ...
                           {filterRecords.map((veiculo, ind)=>(
                              <Grid item xs={12} md={4} key={ind}>
                                 <Box className={classes.box} key={veiculo.Codigo}>
                                    <Link to={`/veiculo/${veiculo.Marca}/${veiculo.Modelo}/${veiculo.Versao}/${veiculo.Codigo}`} className={classes.link}>
                                       {
